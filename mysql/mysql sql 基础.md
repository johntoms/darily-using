...
create table `<tbname>` (id int primary key auto_increment,name varchar(50) not null ,age int default 0 ,btime datetime);
...
1. 声明列类型
  三个类型
  	数字 int
  	字符 char(m) varchar(m) m必须有，代表最大的字符个数
2. 常见约束
   not null 不能为空
   	default 值 设置默认值
...
1. 增 insert into
   ...
   insert into `<tbname>` set col1=`<value>`,col2=`<values>`,....;
   insert into`<tbname>` (col1,col2,col3) values (value,value,value...);
   insert into t1 (col3,col2,col1) values (3,2,1)
...
update `<tbname>` set col1=value,col2=value,... where `<条件>`;
...
命令的用法
grant `<priv>` on `<database>`.`<tbname>` to `<username>`@`<host>` identified by `<password>`;
flush privileges; 手动刷新


grant all on *.* to booboo@'%' identified by 'zyadmin';
...
课堂练习：
创建一个本地的php用户，密码为php123，只拥有对ecshop库中所有的表的所有的all权限。
...
收回权限
语法：
revoke `<priv>` on `<db>`.`<tb>` from `<username>`@`<host>`;

课堂练习：
回收本地的php用户所用于ecshop库中所有的表的的all的权限。
revoke all on ecshop.* from php@'localhost';
